...
**Set Type**:
A curated, fixed label for what one set *is* — **warm-up**, **working**, **drop**, **failure**, or **AMRAP** — drawn from a closed set (never AI- or user-invented, the same species as Training Type and Progression Scheme). Optional and **descriptive only**: an unset Set Type resolves to **working**, so a plain set needs no choice, and Set Type **never feeds Progression** (schemes still read the rep grammar — an `AMRAP` label is not the `"5+"` rep target, nor Greyskull's AMRAP final set). Carried per Exercise Prescription on the *plan* and per Logged Set on the *record* (ADR-0065). Its one *intended* behavioural effect is on read-time analytics — a **warm-up** set is excluded from Volume and from strength records (Estimated 1RM / Personal Record / Top Set), while XP, Completion Outcome, Streak, and Logged Count stay type-neutral — but that exclusion is a deliberate **follow-up**: the v1 foundation (#449) makes Set Type a persisted, round-tripping, badge-rendered annotation only, and every projection still reads type-neutrally until the analytics slice lands.
