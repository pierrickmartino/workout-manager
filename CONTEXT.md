...
**Muscle Atlas**:
The body-map presentation of Muscle Group Coverage on the Analytics screen (ADR-0073): a front/back silhouette whose six real-group regions are heat-shaded by recent volume over the same fixed, range-independent window, where selecting a region reveals its mapped set count and the exercises behind it, with a text list of all six groups (and any Unclassified work) always visible beneath. A *presentation* of the Coverage read, not a new signal — so it inherits Coverage's rules exactly: descriptive only (an untrained region reads neutral, never a nudge), presence + volume never a ranking, Unclassified disclosed but never a body region. Each real group has one distinct palette hue so a region's color identifies its group consistently across the atlas, the Muscle Split, and the Muscle Balance. Its per-group set count counts a set once per distinct group it trains (a presence/volume read), distinct from the even-split, sum-to-100 Muscle Split distribution.
