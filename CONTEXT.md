...
**Catalog**:
The shared, global set of all Exercises — one movement definition per normalized name (ADR-0002), owned by no user and reused across everyone: an AI-invented or user-typed movement is stored once for all. Its entries span every Provenance (curated / AI-generated / user-entered) and every Catalog Completeness tier (Stub / Listable / Enriched), and membership is never deleted. Users **Browse the Catalog** to discover Exercises and open Exercise Detail — a read-only act that never edits it. Distinct from a Protocol or Session, which are user-owned plans that *reference* Catalog Exercises. Membership is never deleted **while referenced**: an unwanted Exercise is **Retired** (hidden from discovery, still resolvable) and only ever hard-deleted when it is both Retired and wholly unreferenced (ADR-0076).
...
**Session Section**:
The part of a Session's arc an Exercise Prescription sits in — **Warm-up**, **Main work**, **Accessory**, or **Cooldown** — that a builder's composition strip groups a Session by (ADR-0074). Like **Movement Pattern**, **Muscle Group**, and **Catalog Completeness**, it is a **read-time projection** — never a stored column, never a migration, never an AI call per read, re-derived for free as a plan is edited — but unlike Movement Pattern (a property of one Exercise in isolation) it is a property of a Prescription *in the ordered context of its Session*, so it is classified over the whole ordered list. The rules, in strength order: an explicit **warm-up** Set Type or the leading run of mobility/prep/cardio movements is **Warm-up**; the trailing run of stretch/breathing (or timed-cardio) movements is **Cooldown**; the first three **compound** movements of the remaining work block are **Main work**; everything else — isolation, or compound movements past the cap — is **Accessory**. Warm-up is only ever leading and Cooldown only trailing, so the four read as contiguous bands in Session order. A movement no rule confidently claims falls to **Accessory** (the neutral supporting-work bucket), never forced into Main work — the honesty of leaving an unmapped muscle **Unclassified**. A **discovery/authoring** axis only: it feeds no generation input, cache key, or Progression, and never gates advancement. Because it is derived, a movement is *influenced into* a Section — reordered to the front/back for warm-up/cooldown, or given a warm-up **Set Type** — never assigned a stored Section label; the builder's composition strip drags a tile by **reordering** it, not by tagging it. Distinct from **Role** (the audience axis — protocol-follower / session-author / admin, ADR-0071): that is *who the person is*, this is *where a movement sits in a workout*. Distinct too from **Set Type** `warm-up`, with which it shares only a word: a Set Type is a per-**set**, record-bearing annotation that leaves working-set analytics, while a warm-up **Section** is a per-**exercise**, plan-only, derived composition band — the two are orthogonal (a main-work lift may open with warm-up **sets**), so a builder that shows the band suppresses the redundant per-row warm-up Set-Type chip rather than merging the concepts.
_Avoid_: Role (that is the audience axis, ADR-0071), Block, Phase (that is a Tempo term), Group (collides with Muscle Group), Segment, category, movement role
...
**Provenance**:
How a catalog Exercise came to exist and how far it can be trusted: `curated` (reviewed by a human, trusted), `ai_generated` (invented by the AI, unvalidated), or `user_entered` (typed by a user when logging an ad-hoc movement with no AI call — the least-validated tier, born with only a name). No **automated** path ever mutates it — Enrichment fills a movement's fields but never changes its Provenance, so AI-filled content on a `user_entered` movement stays `user_entered` and never masquerades as human-reviewed. The **one** path that changes it is a deliberate **admin** act: an admin (the curator) may promote a reviewed movement to `curated`, or correct/demote one, as an explicit, audited action — never a side effect of editing the Exercise's other fields (ADR-0075). Carried on every Exercise so unvalidated content can be flagged, audited, and later enriched, merged, or corrected — important given the domain's caution around injury, rehab, and postpartum cases. A different axis from Catalog Completeness, which is *content presence*, not *trust*.
_Avoid_: Source, origin, verified flag

**Catalog Completeness**:
Whether a catalog Exercise meets the shared quality bar, expressed as a **read-time projection** over its populated fields — never a stored flag — in three states: **Stub** (name only), **Listable** (has a description, a non-empty targeted-muscle list, and at least one Execution Step), and **Enriched** (additionally carries the Primary/Secondary split, difficulty, precautions, and an Exercise Image). Measured **provenance-blind** — a `curated`, `ai_generated`, or `user_entered` Exercise is held to the same yardstick, so even a curated seed can read as sub-bar. A distinct axis from Provenance, which is *origin and trust*, not *content presence*. An **internal/ops axis, not a user-facing signal** (ADR-0041, revised for issue #7): the tiers never render on a user-facing catalog/library/detail surface and the field is absent from those API responses. Internally it stays load-bearing — the `curated → completeness → name` catalog **ranking** key and the input that tells **Enrichment** which movements are sub-bar — and it surfaces to people only as an aggregate catalog-health readout in the **admin** Catalog Enrichment section.
_Avoid_: Quality score, completeness flag/column, verified (that is Provenance), rating, completeness badge (it is not a user-facing badge)
...
**Retire**:
The **admin** act of hiding a catalog Exercise from discovery without destroying it — a **reversible soft tombstone** (ADR-0076). A Retired Exercise disappears from every *discovery / candidate* surface — Browse the Catalog, the Library pick widget, equipment facets, Substitution candidates, an Exercise Detail's variations/alternatives, and the Enrichment scan — yet stays **fully resolvable by id**, so every Exercise Prescription, Logged Set, and Relationship that already references it is untouched (references are by id, and settled record is never rewritten — ADR-0020). Only an admin retires, and only an admin **un-retires** (a clean restore to discovery); retirement is never a side effect of any other path, and an AI-generated or user-typed movement whose name collides with a Retired one **reuses that row without resurrecting it**. Distinct from a hard **catalog delete** — the permanent removal of an Exercise that is *both Retired and wholly unreferenced* (retire-then-delete), the catalog counterpart of a Session **Delete**'s no-Logged-Session guard (ADR-0063). The one act that removes an Exercise from users' view; the shared, curator-facing cousin of the per-user **Delete** and **Remove**.
_Avoid_: Delete (that is removing a standalone Session, and the unreferenced-only hard delete of an Exercise), Remove (that is withdrawing an Exercise Prescription from a Session), Archive, Deactivate, hide (bare), soft-delete
...
**Progression**:
The deterministic, no-AI adjustment of an Exercise Prescription on the user's own copy, computed **read-time** from Logged Sets, leaving the cached artifact untouched. It is governed by the Prescription's chosen **Progression Scheme** (defaulting to **Double Progression**); the term names both the *act* of stepping and, loosely, that default scheme. Two rules hold across **every** scheme, never a per-scheme choice: it **never auto-swaps** a movement — at a pure-bodyweight rep ceiling it *suggests* a harder **Variation** rather than growing reps without bound, and the swap stays a user-initiated **Substitution**; and it never reads a clock (there is no calendar — ADR-0001). The primary mechanism by which recommendations adjust over time.
...
**My Sessions**:
The user's personal library of their **own standalone Sessions** — the destination, reached from **Train**, where saved workouts are **searched** (case-insensitive over **Session Name**, the derived fallback label — so an unnamed Session, and any Session's creation date, is still findable — and **Training Type** in v1), filtered by **Favorite or a single Training Type** (a single-select chip row, the Training-Type chips drawn only from the types actually present), and reopened to run again. Each row also carries its **Exercise Prescription count** (the plan's "N exercises") and its **Logged Count** (the performances — so a user spots at a glance which Sessions they have already trained), shows a row title of the **Session Name** or, when unnamed, the creation date alone (the Training Type reads off the row's badge, not the title), and offers **Delete** on a Session with no logged training. Favoriting is toggled in place from the row. A read surface over the user's owned standalone Sessions only — never Protocol-member Sessions or Protocols — and it lists *plans*, never records. Distinct from **Browse the Catalog** (movement discovery, not the user's plans) and from **History** (the record side).
_Avoid_: Library (that is the Catalog pick-mode widget), My workouts, Saved workouts, Session list

**Recent Sessions**:
A **read-time projection** on **Train** of the user's up to **five** most-recently-**performed** standalone **Sessions**, each offering a one-tap **Start** (straight into a **Live Session**). Ordered by performance recency and **deduped to distinct plans**: the same Session performed repeatedly appears once, at its newest performance. Drawn from the **record** (**Logged Sessions**, newest `performed_on` first) but listing **plans** — a **plan-less** record is skipped (it has no plan to Start; it offers **Capture** from History instead), and a **Protocol-member** Session is excluded (standalone-only, like **My Sessions**). Each row previews the plan's first three **Exercise Prescriptions** — the *plan*, never the last record, because Start runs the plan. The proactive, Train-side cousin of **Repeat**; never a stored list.
...
**Logged Count**:
A **read-time projection** over the *record*: the number of **Logged Sessions** (performances) a user has recorded against one of their Sessions — counted across **every Completion Outcome** (an Incomplete performance is still logged training, the same basis Streak and XP count on), never a stored counter (ADR-0018). Surfaced on each **My Sessions** row to let a user identify at a glance which Sessions they have already trained, and it is the exact fact the **Delete** guard reads — a Session is deletable iff its Logged Count is zero. Counts *performances*, not **Logged Sets** (which would over-count a single workout). A plan-less Logged Session carries no Session id and so contributes to no Logged Count.
_Avoid_: Session count, times performed (loosely), history count, log total
...
**Favorite**:
A **stored, per-user** marker flagging one of a user's own standalone Sessions as a favourite, used to **filter My Sessions**. A user *preference* — the same species as a **Progression Scheme** selection or an **Interface Preference**, which the domain deliberately stores — **not** a read-time projection: the no-stored-ledger rule (ADR-0018) governs *derived* facts (XP, Streak, PRs), never user choices. Per-user and **per-copy**: never carried across **Duplicate** or **Redeem**, so a redeemed copy starts un-favorited for its new owner.
_Avoid_: Star, bookmark, like, saved (bare)

**Author**:
The **human who first created** a Session's plan — surfaced on the Session as its author ("by <name>"). A distinct axis from both the **Owner** (the user who currently holds the row, `clerk_user_id`, which **transfers on Redeem**) and **Session Provenance** (`ai_generated` vs `user_authored` — *how* the plan was made, not *who* made it). **Immutable origin**: preserved across **Duplicate** and **Redeem** and through any re-share chain, so even a heavily edited copy still credits its original creator — the same non-re-attribution as Provenance and `trace_id` lineage. Backfills to the owner for pre-existing rows; a missing display name falls back to a generic label. **The byline is provenance, not self-repetition**: it surfaces only when the Author is **not** the viewing owner (an adopted/shared copy). On a plan the viewer authored themselves it is suppressed — a "by <you>" on every My-Sessions row and the Session view carries no information. The raw Author id stays server-side; the read exposes a derived `authored_by_me` flag (owner == author) in its place, and the client hides the byline when it is true.
...
**Muscle**:
A curated, canonical **individual** muscle — Quadriceps, Latissimus Dorsi, Rectus Abdominis… — that a catalog Exercise's free-form targeted muscles resolve to, the finer tier **beneath** Muscle Group: each Muscle nests under exactly one of the six real Muscle Groups, so the group roll-up (Muscle Split, Muscle Balance, Full-Coverage achievement, and the Coverage read) keeps working unchanged on top. The vocabulary is a curated set of ~40 muscles with a curated free-form→Muscle map, read case- and whitespace-insensitively exactly as the Muscle Group roll-up is; a string that names no known muscle — unknown, AI-invented, or a **bare region term** that names a group rather than one muscle ("back", "deltoids") — falls into an explicit **Unclassified** bucket rather than being guessed at, the honesty twin of the group tier's Unclassified. A **read-time projection** of the existing free-form union, never a stored column and never an AI call per read (same species as Muscle Group and Movement Pattern). The per-muscle vocabulary the anatomical Muscle Atlas reads; distinct from the coarse six-bucket **Muscle Group** it rolls up into.
_Avoid_: Muscle group (that is the coarse roll-up tier), body part, region
...
**Muscle Atlas**:
The **detailed anatomical body-map** presentation of Muscle Group Coverage on the Analytics screen (ADR-0073, extended by ADR-0079): a front/back figure whose **individual Muscles** (the ~40 of the **Muscle** vocabulary, each keyed to a canonical id) are heat-shaded by emphasis-weighted volume over the same fixed, range-independent window, where selecting a muscle reveals its parent group, its mapped set count, and the exercises behind it, with a **two-level text list** (the six groups, each expandable to its muscles) always visible beneath so the section is legible without the illustration. The **figure is chosen by the Fitness Profile's gender** — male / female / a **neutral** fallback for a null or unrecognized value — over one figure-agnostic muscle-id contract. A *presentation* of the Coverage read, not a new signal — so it inherits Coverage's rules exactly: descriptive only (an untrained muscle reads as a neutral faint outline, never a nudge), presence + volume never a ranking, Unclassified disclosed (a neutral footnote) but never a body region. Each real group keeps one distinct palette hue, so a muscle's color identifies its group consistently across the atlas, the Muscle Split, and the Muscle Balance; nothing rides on color, as every muscle and row carries a composed aria-label naming state and volume. Its per-group set count counts a set once per distinct group it trains (a presence/volume read), distinct from the even-split, sum-to-100 Muscle Split distribution. The **six-group roll-up is retained** as the text list's top level and each muscle's parent — the coarse tier survives beneath the finer map.
_Avoid_: Body map (as the stored/UI term), heatmap (bare), 3D body, anatomy chart
...
**Movement Pattern**:
A coarse, curated bucket describing *how a movement moves* — **Squat, Hinge, Push, Pull, Carry, Locomotion, or Core** — that a catalog Exercise rolls up into, with an explicit **General** bucket for a movement no pattern confidently fits. It reorganizes **Browse the Catalog** into a **field-guide taxonomy** (grouped, illustrated sections) rather than a flat name list, and gives each family one consistent line illustration so the library reads visually without a bespoke drawing per movement (ADR-0072). Like **Muscle Group** and **Catalog Completeness**, it is a **read-time projection** — classified from an Exercise's name (the strongest signal) with a conservative targeted-muscle fallback — never a stored column and never an AI call per read; a movement re-classifies for free as Enrichment fills its name or muscles. A pattern is asserted only when it accurately fits (else **General**), the same honesty as leaving an unmapped muscle **Unclassified**. Orthogonal to **Muscle Group**: pattern is the *movement shape* (a Bench Press and an Overhead Press are both **Push**), Muscle Group is the *body region* the muscles roll up to. A **discovery/presentation** axis only — it feeds no generation input, cache key, or progression, so it never reaches a plan the AI conditions on.
_Avoid_: Movement (bare — that is the Exercise concept; this is the *pattern* a movement follows), category, movement family (as the stored term), exercise type
