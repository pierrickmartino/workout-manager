// Original line-art glyphs for the field-guide Movement Pattern taxonomy (ADR-0072), one
// per broad pattern. A deliberately small, consistent set — visual variety across the
// whole Catalog without a bespoke drawing per movement. Each is a minimal figure caught
// mid-pattern on a common 48×48 grid, drawn in `currentColor` so it inherits the entry's
// ink and re-themes for free under every Skin. `general` is the neutral bucket, shown when
// the backend asserts no specific pattern.

import type { MovementPattern } from "@/lib/movement-pattern";

interface MovementGlyphProps {
  pattern: MovementPattern;
  className?: string;
  // Decorative by default (the section label carries the meaning); pass a title to
  // announce it to assistive tech (e.g. the detail hero).
  title?: string;
}
...
// Each pattern's pose as a fragment on the 48×48 grid. Heads are circles; limbs are
// polylines; implements (bars, weights) are the straight segments. Legible from the ~20px
// row glyph up to the 44–64px section and detail-hero sizes.
const POSES: Record<MovementPattern, React.JSX.Element> = {
  squat: (
    <>
