...
// view-model, so this action is a thin transport shell. On success the new
// logged session lands in History (like any performance); on failure the boundary's
// message is surfaced back to the form. The client receives the destination so it can clear
// its acknowledged durable draft before navigating. The payload is the authority — the
// client-side validation is UX, the server re-validates and can still reject.
export async function submitAuthorSession(
  ...
  if (!result.success || !result.data) {
    return {
      error: result.error ?? "Could not save your workout.",
      redirectTo: null,
    };
  }
...
// a thin transport shell. It authors **only** the plan — no second performance
...
export async function submitAuthorPlan(
  ...
  if (!result.success || !result.data) {
    return {
      error: result.error ?? "Could not save this as a reusable session.",
      redirectTo: null,
    };
  }

  return { error: null, redirectTo: `/sessions/${result.data.id}` };
}
