...
A session's **Top Set** for an Exercise is the best Estimated 1RM among its absolute-Load
sets in the trustworthy 1–12-rep window — qualified through the *one yardstick*
(``estimated_1rm_for_set``) the Personal Record tile and PR detector already compare on
(ADR-0017). ``top_set_series`` projects a Logged-Session history onto one Exercise's
...
def _session_top_sets(session: LoggedSessionView) -> dict[int, float]:
    ...
    Only absolute-Load sets in the trustworthy rep window qualify (the one yardstick,
    ``estimated_1rm_for_set``); an Exercise absent from the result had no qualifying set
    that session. The single per-session pass both ``top_set_series`` and
...
def rank_qualifying_exercises(
    ...
    A qualifying Exercise has at least one absolute-Load set in the trustworthy rep window;
    non-qualifying types (bodyweight, qualitative, %-1RM, range, high-rep) never appear.
