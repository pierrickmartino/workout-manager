...
PUT accepts either facet independently: an omitted field keeps the user's current
value, so the Mode picker and the Keep-Screen-Awake toggle each save only their own
facet without disturbing the other. The route reads the current preference, applies
the provided facets, and upserts the whole value.
...
class AppearanceUpdateRequest(BaseModel):
    ...
    Both facets are optional so each control saves only its own: an omitted field
    leaves the user's current value untouched. ``mode`` is typed as the closed
    ``Mode`` enum and ``keep_screen_awake`` as a bool, so anything ill-typed fails
    at the boundary with a 422 and is never persisted — the store only ever holds
    a valid, deliberate choice."""
...
def upsert_appearance(
    ...
    # preference, then upsert the whole value — so Mode and Keep Screen Awake are
    # independently settable and neither save disturbs the other facet.
    current = repo.get_preference(clerk_user_id)
    updated = current.with_overrides(
        mode=payload.mode, keep_screen_awake=payload.keep_screen_awake
    )
