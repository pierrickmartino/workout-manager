...
per Logged Set on the *record*, and in v1 is **descriptive only**:
...
- It **never feeds Progression** — schemes keep reading the rep grammar; an ``AMRAP``
  label is not the ``"5+"`` rep target. This module deliberately holds *no* stepping or
  analytics logic; it only names the members and the two boundary resolutions.
...
stored value reads as ``working``, staying total).
"""
...
class SetType(str, Enum):
    ...
    reads as a working set. ``WARM_UP`` is the one member with a (future) analytics
    consequence — a warm-up leaves working-set Volume and strength records — but that
    filter is a follow-up; here every member is a descriptive label.
    """
