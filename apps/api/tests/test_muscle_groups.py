...
class _EmphasisSet:
    ...
    alongside the flat targeted-muscle union — the shape ``emphasis_of`` reads."""
...
def test_emphasis_of_returns_the_asserted_split_when_one_is_populated():
...
def test_emphasis_of_falls_back_to_all_primary_when_no_split_is_asserted():
...
def test_emphasis_of_keeps_a_primary_only_isolation_split():
...
def test_emphasis_of_an_empty_set_is_empty():
    # Arrange / Act — a set with no muscles recorded at all
    emphasis = emphasis_of(_EmphasisSet())
